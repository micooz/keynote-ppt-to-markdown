...
exports.extractNotesFromPptx = extractNotesFromPptx;
 ...
 */
async function extractNotesFromPptx(pptxFilePath, outputDir) {
    ...
    let zip;
    try {
        zip = new adm_zip_1.default(pptxFilePath);
    }
    catch (e) {
    ...
    if (presentationRelsDoc.Relationships &&
        ...
        console.warn('警告：在 ppt/_rels/presentation.xml.rels 中未找到 Relationships 或 Relationship 元素。');
    }
    let slideCounter = 0;
    for (const sldIdEntry of sldIdLstNode) {
        ...
        // 获取当前幻灯片的图片文件名
        const imageFileName = imageFiles[slideCounter - 1] ||
            `${String(slideCounter).padStart(3, '0')}.png`;
        const imagePath = `images/${imageFileName}`;
        if (!slideRidArray ||
            !Array.isArray(slideRidArray) ||
            slideRidArray.length === 0) {
            console.warn(`警告：幻灯片 ${slideCounter} 的 sldIdEntry 缺少有效的 r:id 数组。内容: ${JSON.stringify(sldIdEntry)}`);
        ...
        if (!slideTarget) {
            console.warn(`警告：未找到 rId 为 ${actualRidString} (幻灯片 ${slideCounter}) 的幻灯片目标。在 slideRelsMap 中未找到。`);
            outputMarkdown += `![](${imagePath})\n\n`;
        ...
        if (slideSpecificRelsEntry) {
            ...
            let notesSlideTargetRelPath = null;
            ...
            if (notesSlideTargetRelPath) {
                ...
                const notesSlidePath = path.posix.join(baseDirForNotesRel, notesSlideTargetRelPath);
                ...
                if (notesSlideEntry) {
                    ...
                    if (pNotesNode) {
                        ...
                        if (cSldNode) {
                            ...
                            if (spTreeNode && Array.isArray(spTreeNode['p:sp'])) {
                                for (const shape of spTreeNode['p:sp']) {
                                    ...
                                    if (shapeType === 'body') {
                                        ...
                                        if (txBodyNode) {
                                            ...
                                            console.warn(`警告：幻灯片 ${slideCounter} 形状标记为 body 但未找到 txBody。`);
                                        }
                                    }
                                ...
                                if (!txBodyNode) {
                                    let foundTxBodyInAnyShape = undefined;
                                    ...
                                    if (countTxBodyShapes === 1) {
                                        ...
                                        console.warn(`警告：幻灯片 ${slideCounter} 在形状中找到 ${countTxBodyShapes} 个 txBody 且无明确标记。将尝试使用第一个。`);
                    ...
                    console.warn(`警告：幻灯片 ${slideCounter} 未找到注释幻灯片文件: ${notesSlidePath}`);
            ...
            console.warn(`警告：幻灯片 ${slideCounter} (目标 ${slideTarget}) 没有找到关联的 .rels 文件 (${slideRelsPath})。假设没有注释。`);
        }
        outputMarkdown += `![](${imagePath})\n\n`;
